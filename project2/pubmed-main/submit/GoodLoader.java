...
public class GoodLoader {
    ...
    // Load data into the Article table
    ...
    // Load data into the Article_Authors table
    ...
    // Load data into the Article_Keywords table
    ...
    // Load data into the Journal table
    ...
    // Load data into the Article_Journal table
    ...
    // Load data into the Publication_Types table
    ...
    // Load data into the Article_Publication_Types table
    ...
    // Load data into the Article_Ids table
    ...
    // Load data into the Article_References table
    ...
    // Load data into the Grant_Info table
    ...
    // Load data into the Article_Grants table
    ...
    /**
     * Updates the Article_Citations materialized view to reflect the latest data.
     * This method should be called after all data has been inserted into the database.
     */
    private static void updateArticleCitations() {
        if (con != null) {
            try (Statement stmt = con.createStatement()) {
                // Refresh the materialized view to update its data
                stmt.executeUpdate("REFRESH MATERIALIZED VIEW Article_Citations");
                con.commit();
                if (verbose) {
                    System.out.println("Article_Citations view updated successfully.");
                }
            } catch (SQLException e) {
                System.err.println("Failed to update Article_Citations view");
                System.err.println(e.getMessage());
                try {
                    con.rollback();
    ...
    public static void main(String[] args) {
        String fileName;
        fileName = "E:\\Subjects\\CS\\Database\\Projects\\pubmed24n.ndjson";
        Properties properties = new Properties();
        properties.put("host", "localhost");
        properties.put("user", "postgres");
        properties.put("password", "/PostgreS@1478950+/");
        properties.put("database", "project2");
...
//             Empty target table
            ...
            // Create the Article_Citations materialized view
            // if (con != null) {
            //     stmt0 = con.createStatement();
            //     // 首先删除已存在的物化视图（如果存在）
            //     stmt0.execute("DROP VIEW IF EXISTS Article_Citations");
            
            //     // 创建物化视图，不使用 'OR REPLACE'
            //     String createMaterializedViewSQL = "CREATE MATERIALIZED VIEW Article_Citations AS " +
            //             "SELECT " +
            ...
            //     stmt0.execute(createMaterializedViewSQL);
            // }
                        // // Update Article_Citations view after all data is inserted
                        // updateArticleCitations();
            ...
            if (con != null) {
                ...
                stmt0.execute("DROP MATERIALIZED VIEW IF EXISTS Article_Citations");
                stmt0.execute("DROP VIEW IF EXISTS Article_Citations");
                stmt0.execute("""
                    ...
                    CREATE TABLE Article_Journal (
                                                    journal_id varchar(20) references journal(id) ON DELETE CASCADE,
                    ...
                    SELECT
                        ar.reference_id AS article_id, -- 被引用的文章 ID
                        COUNT(*) AS citation_count, -- 被引用的次数
                        EXTRACT(YEAR FROM a.date_created) AS citation_year -- 引用发生的年份
                    FROM article_references ar
                    JOIN Article a ON ar.article_id = a.id -- 引用文章的创建日期
                    GROUP BY ar.reference_id, EXTRACT(YEAR FROM a.date_created)
...
//------------------------------------------------------------------------------------add reference
                ...
                if(cnt % 1000 == 0){
                    ...
                    System.out.println(cnt+" "+(cnt * 1000) / (current - start)+" records/s");
                }
            ...
            System.out.println("Total Loading time : "+(end-start)*1.0/1000+"s");
